...
const name=document.querySelector("#name").value.trim(),gender=document.querySelector("#gender").value,theme=document.querySelector("#theme").value.trim(),customText=document.querySelector("#customText").value.trim(),type=document.querySelector("#type").value,style=document.querySelector("#style").value,bg=document.querySelector("#background").value,ratio=document.querySelector("#ratio").value;
const dear=gender==="female"?"היקרה":"היקר";
const greeting=customText || `ל${name} ${dear}, יום הולדת שמח!`;
...
out.value=`צור תמונה ביחס ${ratio} של ${labels.type[type]} עבור ${name}. ${labels.style[style]}.${themeText}${cakeRule} על היצירה יופיע בצורה ברורה וקריאה הכיתוב: "${greeting}". ${labels.bg[bg]}. הקפד שהיצירה המרכזית תופיע בשלמותה, ללא חיתוך, עם קומפוזיציה נקייה וחגיגית.`;
