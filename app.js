...
form.addEventListener("submit",e=>{e.preventDefault();const name=document.querySelector("#name").value.trim(),gender=document.querySelector("#gender").value,theme=document.querySelector("#theme").value.trim(),type=document.querySelector("#type").value,style=document.querySelector("#style").value,bg=document.querySelector("#background").value,ratio=document.querySelector("#ratio").value;
...
out.value=`צור תמונה ביחס ${ratio} של ${labels.type[type]} עבור ${name}. ${labels.style[style]}.${themeText}${cakeRule} על היצירה יופיע בצורה ברורה וקריאה הכיתוב: "ל${name} ${dear}, יום הולדת שמח!". ${labels.bg[bg]}. הקפד שהיצירה המרכזית תופיע בשלמותה, ללא חיתוך, עם קומפוזיציה נקייה וחגיגית.`;
